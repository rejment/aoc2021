...
public class Day24 {
    ...
    public static String csolve(List<String> ins, long[] reg, int ip, int dir) {
        String key = Arrays.toString(reg) + "|" + ip + "|" + dir;
        if (cache.containsKey(key)) {
            return cache.get(key);
        }
        String res = solve(ins, reg, ip, dir);
        cache.put(key, res);
        return res;
    }
        ...
        switch (args[0]) {
            case "inp" -> {
                for (int i = (dir > 0 ? 1 : 9); (dir > 0 ? i <= 9 : i >= 1); i += dir) {
                    ...
                    String res = csolve(ins, r2, ip + 1, dir);
                    if (res != null) {
                        return i + res;
                    }
                }
            ...
            case "add" -> {
                reg[args[1].charAt(0) - 'w'] += arg2(reg, args);
            }
            case "mul" -> {
                reg[args[1].charAt(0) - 'w'] *= arg2(reg, args);
            }
            ...
            case "eql" -> {
                reg[args[1].charAt(0) - 'w'] = reg[args[1].charAt(0) - 'w'] == arg2(reg, args) ? 1 : 0;
            }
            default -> {
